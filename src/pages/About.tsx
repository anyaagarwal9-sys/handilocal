  ...
  return <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="relative grid md:grid-cols-2 gap-8 md:gap-12 items-start">
            ...
            <motion.div className="text-left" initial="initial" animate="animate" variants={staggerContainer}>
              <motion.span variants={fadeInUp} className="inline-block px-4 py-1.5 bg-primary/10 text-primary rounded-full text-sm font-medium mb-4">
                Our Story
              </motion.span>
              <motion.h1 variants={fadeInUp} className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 text-foreground">
            ...
            <motion.div className="text-left md:pl-8 md:border-l border-border" initial={{
            opacity: 0,
            x: 20
          }} animate={{
            opacity: 1,
      ...
      <section className="py-4 px-4 bg-background">
        <div className="container mx-auto max-w-6xl">
          ...
          <motion.div className="grid md:grid-cols-2 gap-8" variants={staggerContainer} initial="initial" whileInView="animate" viewport={{
          once: true
        }}>
            {teamMembers.map(member => <motion.div key={member.name} className="bg-card rounded-3xl p-8 border border-border/50 hover:border-primary/30 hover:shadow-xl transition-all flex flex-col sm:flex-row gap-6 items-center sm:items-start" variants={fadeInUp} whileHover={{
            y: -5
          }}>
                <div className="w-32 h-32 rounded-2xl bg-primary/10 flex items-center justify-center flex-shrink-0 overflow-hidden">
                  {member.image ? <img src={member.image} alt={member.name} className="w-full h-full object-cover object-[50%_22%]" loading="lazy" /> : <User className="w-16 h-16 text-primary/50" />}
                </div>
